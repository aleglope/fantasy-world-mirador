...
//#define BG_ALPHA
//#define SHADOW_ALPHA
// Fantasy World mod: usamos ONLY_BOX en vez de SHADOW_ALPHA. El piso virtual
// del shader proyecta una sombra que, al renderizarse el cubo en screen-space
// reducido (main() lo encoge con monoScale), queda como una estela diagonal
// oscura desproporcionada que tapa el paisaje. ONLY_BOX hace alpha=0 fuera
// de la geometría del box → composición limpia sobre iChannel0.
#define ONLY_BOX
//#define NO_SHADOW
...
#ifdef SHADOW_ALPHA
    // Atenuación de la sombra del box (Fantasy World mod): el factor 1.25 del
    // shader original satura el alpha en una zona muy amplia alrededor del
    // box, lo que tapa el paisaje (iChannel0) cuando el box rota. 0.7 deja
    // la sombra perceptible pero permite ver el fondo a través de ella.
    alpha=clamp(alpha*(1.-aofac)*0.7,0.,1.);
#endif
...
#if defined(BG_ALPHA)||defined(ONLY_BOX)||defined(SHADOW_ALPHA)
    // Fantasy World mod: muestreamos iChannel0 con gl_FragCoord (coord real
    // de pantalla) en vez de fragCoord, porque main() puede transformar
    // fragCoord para reposicionar el monolito. iChannel0 (el paisaje) debe
    // componerse en la posición real del píxel, no en la transformada.
    fragColor.rgb=fragColor.rgb*fragColor.w+texture(iChannel0, gl_FragCoord.xy/iResolution.xy).rgb*(1.-fragColor.w);
#endif
}
